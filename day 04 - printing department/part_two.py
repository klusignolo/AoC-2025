...
did_find_available_rolls = True
total_rolls_to_remove = 0
while did_find_available_rolls:
    new_toilet_paper_grid = [row.copy() for row in toilet_paper_grid]
    ...
    for row_index, row in enumerate(toilet_paper_grid):
        for col_index, col in enumerate(row):
            ...
            if total_toilet_papers_adjacent < 4:
                new_toilet_paper_grid[row_index][col_index] = "."
    ...
    if total_available_toilet_papers == 0:
        did_find_available_rolls = False
    else:
        total_rolls_to_remove += total_available_toilet_papers
        toilet_paper_grid = [row.copy() for row in new_toilet_paper_grid]

print(total_rolls_to_remove)